%###################################################################################################
%NAME    :SetInputFile.m
%PURPOSE :Set/load file name and other parameters.
%DATE    :17/07/2017
%VERSION :
%
...
function [parfile]= SetInputFile(InputFileName, parfile, UseDefault)
  if UseDefault
    ...
    %
    % [Weight Matrix]
    parfile.FileNameMatrixB       ='T2T2_B.dat';
    % [Inversion Points]
    parfile.nx=80;
    parfile.ny=80;
    ...
    while(1)
      ...
      if(strfind(stringa, 'FileNameMatrixB       =')==1) 
          parfile.FileNameMatrixB=strrep(stringa,'FileNameMatrixB       =','');
      end
      %
