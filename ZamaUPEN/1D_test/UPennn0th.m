function [xpwL2_nn,errML2,resML2,Q,INd_exit,ck,Lam,it_pn,time]= UPennn0th(A,b,xex,noise_norm,beta_0,Kmax,tol_lam)
    %
    %  UPenMMmL2 non neg Inexact
    %
    % Set up parameters
    [N,M]=size(A);
    L2 = get_l(M,0); %change for 0th order (M, 0), for 1st (M+1, 1); simulation co
    % L2(:,1)=[];
    % L2(:,end)=[];
