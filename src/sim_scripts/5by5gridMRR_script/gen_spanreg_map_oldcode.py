...
# noise_file_path = "/home/kimjosy/LocReg_Regularization-1/SimulationsSets/MRR/SpanRegFig/est_table_SNR100_iter1_lamini_LCurve_dist_narrowL_broadR_newLR_01Oct24noise_arr.npy"

# noise_file_path = "/home/kimjosy/LocReg_Regularization-1/SimulationsSets/MRR/SpanRegFig/est_table_SNR1000_iter1_lamini_LCurve_dist_narrowL_broadR_500maxiter_03Oct24noise_arr.npy"
...
noise_file_path = "/home/kimjosy/LocReg_Regularization-1/SimulationsSets/MRR/SpanRegFig/est_table_SNR1000_iter50_lamini_LCurve_dist_narrowL_broadR_SNR1000_final_09Oct24noise_arr.npy"
...
feedback = True
lam_ini_val = "LCurve"
dist_type = f"narrowL_broadR_testing_noparallel"
# gamma_init = 5
...
def calc_T2mu(rps):
    ...
    nrps = len(rps)
    # T2_right = 140 * np.ones(nrps)
    # T2_left = 140 * np.ones(nrps)
    # T2_right = T2_left / rps

    # # T2_left = T2_right/rps
    # T2mu = np.column_stack((T2_right, T2_right))
...
def generate_estimates(i_param_combo):
    ...
    print("i_param_combo", i_param_combo)
    print("type(i_param_combo)", type(i_param_combo))
    ...
    if preset_noise == False:
        ...
        # print("dat_noisy", dat_noisy[0:10])
   ...
   # f_rec_LocReg_LC, lambda_locreg_LC, test_frec1, test_lam1 = LocReg_Ito_mod(dat_noisy, A, LRIto_ini_lam, f_rec_ini, gamma_init, maxiter)
    ...
    # if peak_test_true == True:
    #     if len(rps) > 1:
    #         raise AssertionError("We only want to test one rps value for overlapping candidates")
...
if __name__ == '__main__':
    ...
    if show == 1:
        ...
        def add_heatmap(ax, data, tick_labels, title, x_ticks, y_ticks):
            ...
            ax.set_xlabel('Ratio of Peak Separation', fontsize=18)
            ax.set_ylabel('Gaussian Sigma', fontsize=18)
            ...
            # cbar = fig.colorbar(im.collections[0], ax=ax, orientation='horizontal', pad=0.04)
        ...
        plt.savefig(os.path.join(file_path_final, f"heatmap.png"))
        print("Saved Heatmap")
        plt.close()
