...
# ref_est = df.iloc[869]["ref_estimate"]
# gcv_est = df.iloc[869]["GCV_estimate"]
# lr_est = df.iloc[869]["LR_estimate"]
...
# print("gcv_MWF", gcv_MWF)
# print("ref_MWF", ref_MWF)
# print("lr_MWF", lr_MWF)
...
# mask_gcv_norm = gcv_norm
# mask_ref_norm = ref_norm
# mask_lr_norm = lr_norm
...
mask_Myelin_idx = np.where((T2 >= 20) & (T2 <= 40))
mask_wass = np.where((T2 < 20) | (T2 > 165))
# mask_gcv_norm[mask_wass] = 0
# mask_ref_norm[mask_wass] = 0
# mask_lr_norm[mask_wass] = 0
...
# total_MWF = np.cumsum(mask_gcv_norm)
# mask_gcv_MWF = total_MWF[mask_Myelin_idx[-1][-1]]
# total_MWF = np.cumsum(mask_ref_norm)
# mask_ref_MWF = total_MWF[mask_Myelin_idx[-1][-1]]
# total_MWF = np.cumsum(mask_lr_norm)
# mask_lr_MWF = total_MWF[mask_Myelin_idx[-1][-1]]
# plt.plot(T2,mask_gcv_norm,label = "GCV");plt.plot(T2,mask_lr_norm,label='LR');plt.plot(T2,mask_ref_norm,label="ref");plt.legend();plt.show()

# print("masked_gcv_MWF", f"{mask_gcv_MWF}")
# print("mask_ref_MWF", f"{mask_ref_MWF}")
# print("masked_lr_MWF", f"{mask_lr_MWF}")
...
def calc_new_MWF(f_rec):
    ...
    # mask_wass = np.where((T2 < 20) | (T2 > 165))
    Myelin_idx = np.where(T2 <= 40)
    # mask_wass = np.where((T2 > 165))
    mask_wass = np.where((T2 < 16) | (T2 > 165))
    f_rec[mask_wass] = 0
...
df.to_pickle("/Users/kimjosy/Downloads/LocReg/results/brain/noise_addition_exp/Sep1925/est_table_xcoordlen_313_ycoordlen_313_NESMA_filtered_NA_GCV_LR012_UPEN19Sep25_modifiedMWF.pkl")