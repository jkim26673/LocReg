...
def LocReg_unconstrained(data_noisy, G, x0_ini, lambda_ini, ep1, ep2, ep3):
    ...
    while True:
        ...
        # dlambda1 = SD_noise * np.sum(np.abs(x0_ini))
        
        # dlambda1 = SD_noise * np.sum(1 / np.abs(x0_ini))
        # dlambda1 = (np.linalg.norm(G.T @ data_noisy)/ (np.linalg.norm(G.T @ G @ G.T) + np.linalg.norm(x0_ini)))/((x0_ini) + ep1)
        # dlambda1 = (np.linalg.norm(G.T @ data_noisy)/ (np.linalg.norm(G.T @ G @ G.T) + np.linalg.norm(G.T @ G @ x0_ini) * np.linalg.norm(x0_ini) + np.linalg.norm(G.T @ G @ x0_ini)**2))/((x0_ini) + ep1)
