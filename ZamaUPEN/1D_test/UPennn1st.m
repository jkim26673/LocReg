function [xpwL2_nn,errML2,resML2,Q,INd_exit,ck,Lam,it_pn,time]= UPennn1st(A,b,xex,noise_norm,beta_0,Kmax,tol_lam)
    %
    %  UPenMMmL2 non neg Inexact
    %
    % Set up parameters
    [N,M]=size(A);
    L2 = get_l(M+1,1); %change for 0th order (M, 0), for 1st (M+1, 1); simulation co
    %L2(:,1)=[];
    L2(:,end)=[];
    ...
    %if exist('sol_exit', 'var')
    %    xpwL2_nn = sol_exit;
    %else
    %    disp('Error: sol_exit was not assigned, check the convergence criteria.');
        % Handle error by setting xpwL2_nn as a zero vector with the same length as x
    %    xpwL2_nn = zeros(size(xex));  % Default to a zero vector of the same size as x
    %end
    xpwL2_nn = sol_exit;
