%########################################################################################
% % Main for figures 1 (c) and 2 (GUpenMM)
%########################################################################################
%########################################################################################
close all
clear all
...
%=======================================================================
  ...
  normexact = norm((Kc*X_true*Kr'-s)/Amp_scale,'fro')^2; fprintf('normexact scal=%e \n',normexact)
%=======================================================================
%
% fista
%
par.Amp_scale=Amp_scale;
par.fista_maxiter = 500000;%50000;
par.fista_tol     = 1.e-7;%1.e-10;%5e-6;
par.fista_crit    = 1;
par.fista_verb_fista = 0;
par.scale_fact  = scale_fact;
par.true        = X_true;
par.upen_tol_x    = 1.e-4;%5.e-2;
par.upen_tol_res    = 1.e-6;% forse inutile
% verbose     
 [x,LAMBDA,hist] = GUpen_2D(Kc,Kr,s,par);Testo='Upen2D-L1';
%%
     fprintf('%s   \n',Testo)
     Err=hist.err(end);
 ...
 fprintf('\n \n res=%e it_fista %d time=%e s. \n\n',hist.res(end),IT_int,sum(hist.tempi));
  ...
  if (FL_typeKernel==1) || (FL_typeKernel==2)
     fprintf('%s - T2=%0.2f T1=%0.2f peak=%0.2f  PercTot=%0.2f  \n',Testo,T2(iy),T1(ix),picco,Perc);
     fprintf('%s (T2= %0.2f %0.2f %0.2f, T1 = %0.2f %0.2f %0.2f) \n',Testo,T2(iy-1),T2(iy),T2(iy+1),T1(ix-1),T1(ix),T1(ix+1));
  elseif (FL_typeKernel==3)
     fprintf('%s T2=%0.2f D=%0.2f peak=%0.2f  PercTot=%0.2f  \n',Testo,T2(iy),T1(ix),picco,Perc);
     fprintf('%s  (T2= %0.2f %0.2f %0.2f, D = %0.2f %0.2f %0.2f) \n',Testo,T2(iy-1),T2(iy),T2(iy+1),T1(ix-1),T1(ix),T1(ix+1));
  elseif (FL_typeKernel==4)
     fprintf('%s T22=%0.2f T21=%0.2f peak=%0.2f  PercTot=%0.2f  \n',Testo,T2(iy),T1(ix),picco,Perc);
     fprintf('%s  (T22= %0.2f %0.2f %0.2f, T21 = %0.2f %0.2f %0.2f) \n',Testo,T2(iy-1),T2(iy),T2(iy+1),T1(ix-1),T1(ix),T1(ix+1));
  end
...
function [x,ck,hist]=GUpen_2D(Kc,Kr,s,par)
%
%
...
% 
px = L1nx*(x(:)); %derivata lungo la direzione x
py = L1ny*(x(:)); %derivata lungo la direzione y
v = sqrt(px.^2+py.^2);
p = reshape(v,nx,ny);
...
% stopping criteria
continua = 1; i=1;cond_res=1;X_init=zeros(size(x0));%x0;
...
% Solve the constrained subproblem
 ...
 %   if continua
        ...
        Rsqrd = norm((Kc*x*Kr'-s),'fro')^2; % Rsqrd= Rsqrd/NN;
        ck =Rsqrd./(NN*(beta_0+beta_p*p.^2+beta_c*c.^2)); 
        ck_true =Rsqrd./((NN+1)*(beta_0+beta_c*c_true.^2)); 
         lambda=Rsqrd/norm(x(:),1);
...
errGupen=[err0 err];
Q_Gupen=Q1;
save dati_conv_GupenMM Q_Gupen errGupen
end
