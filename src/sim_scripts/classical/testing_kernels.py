...
# A = np.array(result[0])
# A,data_noiseless,g = baart(n = 100)
# A, data_noiseless, g = gravity(n = 100, nargin=1)
# A, data_noiseless, g = phillips(n = 100, nargout=3)
# A, data_noiseless, g = shaw(n = 100, nargout=3)
A, data_noiseless, g = wing(n=100, nargin=1, nargout=3)
b_exact = np.array(result[4]).flatten()
dat_noisy = np.array(result[1]).flatten()
noise = dat_noisy - b_exact
# xex = np.array(result[2]).flatten()
xex = g
# noise_norm = np.array(result[3]).flatten()
noise_norm = np.linalg.norm(noise)
b_exact = data_noiseless
dat_noisy = data_noiseless + noise
...
def runs(numNRs):
    ...
    for iter_sim in tqdm(range(numNRs)):
        ...
        realresult = 1
        N = len(xex)
        T2 = np.linspace(-np.pi/2,np.pi/2,100)
        TE = T2
