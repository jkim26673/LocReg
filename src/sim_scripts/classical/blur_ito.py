...
def minimize_OP(Alpha_vec, L, data_noisy, G, nT2, g):
    OP_x_lc_vec = np.zeros((nT2**2, len(Alpha_vec)))
...
if np.any(g) < 0:
    ...
    print("blur is non-neg problem")


# Umat = scipy.io.loadmat('/Users/steveh/Downloads/blur_Umat.mat')
# U = Umat['U']

# Vmat = scipy.io.loadmat('/Users/steveh/Downloads/blue_Vmat.mat')
# V = Vmat['V']
...
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Blur Problem No Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
...
#         table.set_fontsize(12)
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Blur Problem with Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
#         plt.tight_layout()
#         string = "blur"
#         file_path = create_result_folder(string, SNR)
...
# date = datetime.now().strftime("%Y%m%d")
# np.savetxt(f'/Users/steveh/Downloads/NIH 23-24/LocReg_Python/Experiments/classical_problems/Testing_Ito_{date}_blur.csv', vecs , delimiter=',')
...
# res_vec_DP = np.zeros((n**2,nrun))
# res_vec_GCV = np.zeros((n**2,nrun))
# res_vec_LC = np.zeros((n**2,nrun))
# res_vec_locreg_DP = np.zeros((n**2,nrun))
# res_vec_locreg_GCV = np.zeros((n**2,nrun))
# res_vec_locreg_LC = np.zeros((n**2,nrun))

# noise_size = int(data_noiseless.shape[0])
# noise_arr = np.zeros((noise_size, nrun))
# lam_LC = np.zeros((n**2,nrun))
# lam_GCV = np.zeros((n**2,nrun))
# lam_LocReg = np.zeros((n**2,nrun))
# lam_DP = np.zeros((n**2,nrun))
# lam_LocReg_ne = np.zeros((n**2,nrun))
...
#     # f_rec_locreg, lambda_locreg = LocReg_unconstrained_NE(data_noisy, G, x0_ini, lambda_LC, ep1, ep2, ep3)
...
# np.savetxt('blur_vec_1121.csv', vecs , delimiter=',')
...
# #         'SNR': SNR
# #     }
# # print("Finished computing blur_info")
# # # Save file
# # FileName = f"blur_info_09_11_23_1000NR"
...
# plt.savefig(f'blur_prob_nrun_{nrun}_{name}.png')  # You can specify the desired filename and format
# plt.show()
...
# dict_of_labels = dict(zip(labels, handles))
# plt.legend(dict_of_labels.values(), dict_of_labels.keys(), fontsize='small', loc='best')
# #plt.legend(fontsize='small'10, loc='upper left')
# #plt.legend(['DP', 'GCV', 'L-curve', 'LocReg'], fontsize=20, loc='best')
# plt.xlabel('T2', fontsize=20, fontweight='bold')
# plt.ylabel('Lambda', fontsize=20, fontweight='bold')
...
# plt.savefig(f'blur_prob_091123_nrun_{nrun}.png')  # You can specify the desired filename and format
# plt.show()
...
# plt.tight_layout()
# plt.savefig('blur_prob_08_22_23.png')  # You can specify the desired filename and format
# plt.show()
