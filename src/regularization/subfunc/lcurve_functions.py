...
def curvature(lambd, sig, beta, xi):
    ...
    # curvature.
    curv = - np.divide((dlogrho * ddlogeta - ddlogrho * dlogeta),(dlogrho**2 + dlogeta**2)**(1.5))
    return curv
...
#     beta = np.matmul((np.conjugate(u)), bm)
#     beta = np.reshape(beta[0:int(p[0])], beta.shape[0])
#     b0 = (bm - (np.matmul(beta.T, u)).T)#u @ beta
#     # s = sig
...
def l_corner(rho, eta, reg_param, u, sig, bm):
    ...
    x2 = reg_param[int(np.amax([curv_id, 0]))]
    reg_c = optimize.fminbound(curvature, x1, x2, args=(sig, beta, xi), full_output=False, disp=False)
    reg_c_index = np.where(reg_param == reg_c)[0][0]  # Get the index of reg_c
    kappa_max = -curvature(reg_c, sig, beta, xi)  # Maximum curvature.
    if kappa_max < 0:
        lr = len(rho)
        reg_c = reg_param[lr]
...
def l_cuve(u, sig, bm, plotit = False):
    ...
    lam_opt_index = np.where(reg_param == lam_opt)[0][0]

    return lam_opt_index
