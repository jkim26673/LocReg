...
print("\n--- 8. Absolute Difference Calculation ---")
absdiff_columns = [f'absdiff_{mwf}' for mwf in mwf_columns_short if mwf != 'MWF_Ref']
for mwf in mwf_columns_short:
    if mwf == 'MWF_Ref':
        continue
...
for mwf in mwf_columns_short:
    if mwf == 'MWF_Ref':
        continue
    mse = np.mean((df_filtered_mwf_outliers["MWF_Ref"] - df_filtered_mwf_outliers[mwf]) ** 2)
    mse_results[mwf] = mse
...
table_data = {
    'MWF Parameter': [mwf for mwf in mwf_columns_short if mwf != 'MWF_Ref'],
    'Mean Absolute Diff': average_absdiffs.values,
    'Std Dev of Abs Diff': std_dev_absdiffs.values,
    'MSE vs MWF_Ref': [mse_results[mwf] for mwf in mwf_columns_short if mwf != 'MWF_Ref']
}
table_df = pd.DataFrame(table_data)
