...
# V_arr = []
# for i in range(nsim):
#     Gtest = np.random.random(100).reshape(10,10)
#     U,s,V = csvd(Gtest, tst = None, nargin = 1, nargout = 3)
#     U_arr.append(U)
#     s_arr.append(s)
#     V_arr.append(V)

# stacked_arrays = np.stack(U_arr)
# average_at_each_position = np.mean(stacked_arrays, axis=0)
...
if np.any(g) < 0:
    ...
    print("baart is non-neg problem")
...
# date = datetime.now().strftime("%Y%m%d")
# np.savetxt(f'/Users/steveh/Downloads/NIH 23-24/LocReg_Python/Experiments/classical_problems/Testing_Ito_{date}_baart.csv', vecs , delimiter=',')
nrun_values = [20]  # Example: vary nrun between 20 and 30
expvalue_values = [0]  # Example: only one value for expvalue
SNR_values = [30]  # Example: vary SNR between 300 and 30
# feedback_values = [False, True]  # Example: vary feedback between False and True
lam_ini_values = ["DP"]  # Example: vary lam_ini between 'LC' and 'other'
...
for i in tqdm(range(nrun)):
    ...
    # lam_LocReg_GCV[:,i] = lambda_locreg
    # com_vec_locreg_GCV[i] = norm(g - f_rec_locreg)
    # res_vec_locreg_GCV[:,i] = f_rec_locreg
...
# print('The mean error for locreg_DP is', str(np.median(com_vec_locreg_DP)))
# print('The mean error for locreg_GCV is', str(np.median(com_vec_locreg_GCV)))
print('The median error for locreg_LC is', str(np.median(com_vec_locreg_LC)))
print('The median error for Ito_NP is', str(np.median(com_vec_ItoLR)))
"""
...
#     com_vec_locreg_GCV[i] = norm(g - f_rec_locreg)
#     res_vec_locreg_GCV[:,i] = f_rec_locreg
#     results =  (com_vec_DP[i], com_vec_LC[i], com_vec_GCV[i], com_vec_locreg_DP[i], com_vec_locreg_LC[i], com_vec_locreg_GCV[i], res_vec_DP[:, i], res_vec_LC[:, i], res_vec_GCV[:, i], res_vec_locreg_DP[:, i], res_vec_locreg_LC[:, i], res_vec_locreg_GCV[:, i])
#     return results
...
#     results = list(tqdm(executor.map(process_iteration, range(nrun))))



# fig, axs = plt.subplots(1, 2, figsize=(12, 6))
#             # Subplot 1: Reconstructions
# axs[0].plot(g, label="Ground Truth", linewidth=6.0)
# axs[0].plot(res_vec_DP[:,0], label="Discrepancy Principle", linewidth=3.0)
# axs[0].plot(res_vec_GCV[:,0], label="GCV", linewidth=3.0)
# axs[0].plot(res_vec_LC[:,0], label="L-Curve", linewidth=3.0)
# axs[0].plot(res_vec_locreg_LC[:,0], label="LocReg", linewidth=3.0)

# # axs[0].plot(f_rec_DP_OP, label = "DP")
# # axs[0].plot(f_rec_LC_OP, label = "LC")
# # axs[0].plot(f_rec_GCV_OP, label = "GCV")
# axs[0].set_xlabel("T2", fontsize = 20, fontweight='bold')
# axs[0].set_ylabel("Amplitude", fontsize = 20, fontweight='bold')
# axs[0].set_title(f"MRR Problem NR{nrun}")
# axs[0].legend(["Ground Truth",  "DP", "GCV", "LC", "LocReg"])

# # Subplot 2: Alpha Plots

# #Print out the OP Alpha
# axs[1].semilogy(T2, lam_LocReg_LC[:,0], linewidth=3, color='orange', label='LocReg')
# axs[1].semilogy(T2, lam_DP[:,0] , linewidth=3, color='blue', label='DP')
# axs[1].semilogy(T2, lam_LC[:,0] , linewidth=3, color='gray', label='LC')
# axs[1].semilogy(T2, lam_GCV[:,0], linewidth=3, color='purple', label='GCV')

# plt.show()
# print('The mean error for locreg is', str(np.median(com_vec_locreg_ne)))
...
# com_vec_DP, com_vec_LC, com_vec_GCV, com_vec_locreg_DP, com_vec_locreg_LC, com_vec_locreg_GCV = zip(*results)
"""
vecs = np.asarray([com_vec_DP, com_vec_LC, com_vec_GCV, com_vec_locreg_DP, com_vec_locreg_LC, com_vec_locreg_GCV])
"""
# np.savetxt('baart_vec_1023.csv', vecs , delimiter=',')
# np.savetxt('baart_vec_1121.csv', vecs , delimiter=',')
# np.savetxt('baart_vec_1206.csv', vecs , delimiter=',')
...
# print('The SD for locreg is', str(np.std(com_vec_locreg_DP)))
# print('The SD for locreg is', str(np.std(com_vec_locreg_LC)))
# print('The SD for locreg is', str(np.std(com_vec_locreg_GCV)))
...
# #         'SNR': SNR
# #     }
# # print("Finished computing baart_info")
# # # Save file
# # FileName = f"baart_info_09_11_23_1000NR"
...
# #     pickle.dump(baart_info, file)
# # print(f"File saved at: {file_path}")

# # file_path = '/Users/steveh/Downloads/NIH 23-24/LocReg_Python/classical_problems/problem_info/baart_info_09_11_23_1000NR.pkl'
# # info = np.load(file_path, allow_pickle=True)
# # res_vec_LC = info['LC']
# # res_vec_GCV = info['GCV']
# # res_vec_locreg = info['LocReg']
# # res_vec_DP = info['DP']
...
# # plt.savefig(f'baart_prob_nrun_{nrun}_{name}.png')  # You can specify the desired filename and format
# plt.show()

# name = "locreg_gt"
# plt.figure(figsize=(12, 6))
# # plt.plot(T2, g, linewidth=3, color='black', label='True Dist.')
# plt.plot(T2, res_vec_locreg_LC, linestyle=':', linewidth=3, color='magenta', label='LocReg')
# plt.legend(fontsize='small', loc='best')
# plt.xlabel('T2', fontsize=20, fontweight='bold')
# plt.ylabel('Amplitude', fontsize=20, fontweight='bold')
# # plt.savefig(f'baart_prob_nrun_{nrun}_{name}.png')  # You can specify the desired filename and format
...
# plt.semilogy(T2, lam_LocReg_ne * np.ones(len(T2)), linestyle='-.',linewidth=3, color='orange', label='LocReg_NE')
...
# #Show the g.t with all methods, G@ methods, and lambda plots
...
# plt.plot(T2, g, linewidth=3, color='black', label='True Dist.')
# # plt.plot(T2, res_vec_locreg, linestyle=':', linewidth=3, color='magenta', label='LocReg')
# plt.plot(T2, res_vec_locreg_DP, linewidth=3, color='red', label='LocReg_DP')
# plt.plot(T2, res_vec_locreg_LC, linewidth=3, color='magenta', label='LocReg_LC')
# plt.plot(T2, res_vec_locreg_GCV, linewidth=3, color='green', label='LocReg_GCV')
# plt.plot(T2, res_vec_DP, linewidth=3, color='brown', label='DP')
# plt.plot(T2, res_vec_GCV, linestyle='--', linewidth=3, color='blue', label='GCV')
# plt.plot(T2, res_vec_LC, linestyle='-.', linewidth=3, color='cyan', label='L-curve')
...
# # plt.plot(T2, G @ res_vec_locreg_ne, linestyle=':', linewidth=3, color='green', label='LocReg_New')
# plt.plot(T2, G @ res_vec_locreg_DP, linewidth=3, color='red', label='LocReg_DP')
# plt.plot(T2, G @ res_vec_locreg_LC, linewidth=3, color='magenta', label='LocReg_LC')
# plt.plot(T2, G @ res_vec_locreg_GCV, linewidth=3, color='green', label='LocReg_GCV')
...
# plt.subplot(1,3,3)
# plt.semilogy(T2, lam_LocReg_DP * np.ones(len(T2)), linestyle=':', linewidth=3, color='magenta', label='LocReg')
# plt.semilogy(T2, lam_LocReg_ne * np.ones(len(T2)), linestyle=':', linewidth=3, color='green', label='LocReg_New')
# plt.semilogy(T2, lam_DP * np.ones(len(T2)),linewidth=3, color='brown', label='DP')
# plt.semilogy(T2, lam_GCV * np.ones(len(T2)), linestyle='--', linewidth=3, color='blue', label='GCV')
# plt.semilogy(T2, lam_LC * np.ones(len(T2)), linestyle='-.',linewidth=3, color='cyan', label='L-curve')
...
# plt.savefig(f'baart_prob_091123_nrun_{nrun}_deleteafter.png')  # You can specify the desired filename and format
# plt.show()
