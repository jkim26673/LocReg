...
if np.any(g) < 0:
    ...
    print("foxgood is non-neg problem")
...
#     if i % 15 == 0:
#         #Plot the curves
#         # lambdasfeed = [LR_Ito_lamsfeed, LR_Ito_lams2feed, LR_Ito_lams3feed, LR_Ito_lams4feed]
#         # reconstfeed = [LR_mod_recfeed, LR_mod_rec2feed, LR_mod_rec3feed, LR_mod_rec4feed]
#         # lambdas = [LR_Ito_lams, LR_Ito_lams2, LR_Ito_lams3, LR_Ito_lams4]
#         # reconst = [LR_mod_rec, LR_mod_rec2, LR_mod_rec3, LR_mod_rec4]
#         # for i in range(len(lambdasfeed)):
#         #     if any(value < 1e-10 for value in lambdasfeed[i]) or any(value < 1e-10 for value in lambdas[i]):
#         #         reconstfeed[i] = 0  # Set corresponding reconstruction index to 0
#         #         reconst[i] = 0  # Set corresponding reconstruction to None or any appropriate value
...
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Foxgood Problem No Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
...
#         table.set_fontsize(12)
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Foxgood Problem with Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
#         plt.tight_layout()
#         string = "Foxgood"
#         file_path = create_result_folder(string, SNR)
...
# date = datetime.now().strftime("%Y%m%d")
# np.savetxt(f'/Users/steveh/Downloads/NIH 23-24/LocReg_Python/Experiments/classical_problems/Testing_Ito_{date}_foxgood.csv', vecs , delimiter=',')
...
np.savetxt('foxgood_vec_1121.csv', vecs , delimiter=',')
# np.savetxt('foxgood_vec_1023.csv', vecs , delimiter=',')
# np.savetxt('foxgood_noise_arr_1000', noise_arr)
...
# # Save file
# FileName = f"foxgood_info_09_11_23_{nrun}NR"
...
plt.savefig(f'foxgood_prob_nrun_{nrun}_{name}.png')  # You can specify the desired filename and format
plt.show()
...
plt.savefig(f'foxgood_prob_nrun{nrun}_09_11_23.png')  # You can specify the desired filename and format
plt.show()
"""