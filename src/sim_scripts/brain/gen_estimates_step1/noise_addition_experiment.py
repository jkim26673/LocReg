...
#             factor = np.sum(sol1) * dT
#             curr_data = curr_data/factor
#             # curr_data, noise = add_noise(curr_data, SNR = SNR_unif_level)
#             # curr_data, noise = add_noise(curr_data, SNR = curr_SNR)
#         #normalize data; check if normalization 1 after identifying weird pixels; seaborn plots;
#         # if unif_noise == True:
#         #     # curr_data = curr_data + unif_noise_val
#         #     # noise = unif_noise_val
...
#         # if np.all(noisy_f_rec_UPEN[:-1] == 0) or all_close_to_zero:
#         #     # print("f_rec_LR is 0")
#         #     noisy_MWF_UPEN = 0
#         #     method = "UPEN"
#         #     Flag_Val = 1
#         #     # curve_plot(method, x_coord, y_coord, f_rec_LR, curr_data, lambda_LR, curr_SNR, MWF_LR, filepath)
#         # else:
#         #     noisy_f_rec_UPEN, noisy_MWF_UPEN = compute_MWF(noisy_f_rec_UPEN, T2, Myelin_idx)
#         #     Flag_Val = 0
...
#         feature_df["curr_data"] = [curr_data]
#         # if unif_noise == True:
#         #     feature_df["unnormalized_data"] = [unnormalized_data]
#         #     feature_df["unnormalized_noise"] = [noise]
#         # else:
#         #     pass
#         feature_df["noise"] = [noise]
#         feature_df["curr_SNR"] = [curr_SNR]
