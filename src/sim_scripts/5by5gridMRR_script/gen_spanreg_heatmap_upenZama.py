...
#UPEN Zama hyperparameters
# Kmax = 500
# beta_0 = 1e-7  # New regularization parameter
# tol_lam = 5e-2  
...
def generate_estimates(i_param_combo, seed=None):
    def plot(iter_sim, iter_sigma, iter_rps):
        ...
        # errors = {'LocReg': err_LR, 'Oracle': err_oracle, 'DP': err_DP, 'GCV': err_GCV, 'L-curve': err_LC, "UPEN_2nd_Der": err_upen, "UPEN_1st_Der": err_upen1D, 
        #           "UPEN": err_upen0D,'LocReg_1st_Der': err_LR1, 'LocReg_2nd_Der': err_LR2} # replace UPEN with "UPEN_Zama"
        errors = {'LocReg': err_LR, 'Oracle': err_oracle, 'DP': err_DP, 'GCV': err_GCV, 'L-curve': err_LC, "UPEN_Zama": err_upen,
                  'LocReg_1st_Der': err_LR1, 'LocReg_2nd_Der': err_LR2} # replace UPEN with "UPEN_Zama"

        # errors = {'Oracle': err_oracle, 'DP': err_DP, 'GCV': err_GCV, 'L-curve': err_LC, "UPEN_Zama": err_upen, 'LocReg_2nd_Der': err_LR2}
        min_method = min(errors, key=errors.get)
        # Modify the plot labels to include a star next to the method with the lowest error
        plt.subplot(1, 3, 1)
        plt.plot(T2, IdealModel_weighted, linewidth=3, color='black', label='Ground Truth')
        locreg_label = f'LocReg (Error: {"{:.2e}".format(err_LR)})'
        if min_method == 'LocReg':
            locreg_label += ' *'
        plt.plot(T2, f_rec_LocReg_LC, linestyle=':', linewidth=3, color='red', label=locreg_label)

        locreg_label1 = f'LocReg 1st Derivative (Error: {"{:.2e}".format(err_LR1)})'
        ...
        locreg2_label = f'LocReg 2nd Derivative (Error: {"{:.2e}".format(err_LR2)})'
        ...
        upen_zama_label = f'UPEN (Error: {"{:.2e}".format(err_upen)})'
        if min_method == 'UPEN_Zama':
            upen_zama_label += ' *'
        plt.plot(T2, upen_zama_sol, linestyle='--', linewidth=3, color='purple', label=upen_zama_label)

        # upen_zama_label = f'UPEN 2nd Derivative (Error: {"{:.2e}".format(err_upen)})'
        # if min_method == 'UPEN_2nd_Der':
        #     upen_zama_label += ' *'
        # plt.plot(T2, upen_zama_sol, linestyle='--', linewidth=3, color='purple', label=upen_zama_label)

        # upen_zama1_label = f'UPEN 1st Derivative (Error: {"{:.2e}".format(err_upen)})'
        # if min_method == 'UPEN_1st_Der':
        #     upen_zama_label += ' *'
        # plt.plot(T2, upen_zama_sol1D, linestyle='--', linewidth=3, color='purple', label=upen_zama_label)

        # upen_zama0_label = f'UPEN (Error: {"{:.2e}".format(err_upen)})'
        # if min_method == 'UPEN':
        #     upen_zama_label += ' *'
        # plt.plot(T2, upen_zama_sol0D, linestyle='--', linewidth=3, color='purple', label=upen_zama_label)
        ...
        plt.plot(TE, A @ f_rec_LocReg_LC, linestyle=':', linewidth=3, color='red', label= f'LocReg')
        plt.plot(TE, A @ f_rec_LocReg_LC1, linestyle=':', linewidth=3, color='green', label= f'LocReg 1st Derivative')
        # plt.plot(TE, A @ f_rec_LocReg_LC2, linestyle=':', linewidth=3, color='crimson', label= f'LocReg 2ndDer {lam_ini_val}')
        # plt.plot(TE, A @ f_rec_LocReg_LC2, linestyle=':', linewidth=3, color='crimson', label= f'LocReg')
        plt.plot(TE, A @ f_rec_LocReg_LC2, linestyle=':', linewidth=3, color='orange', label= f'LocReg 2nd Derivative')
        ...
        plt.semilogy(T2, (lambda_locreg_LC) * np.ones(len(T2)), linestyle=':', linewidth=3, color='red', label=f'LocReg')
        plt.semilogy(T2, (lambda_locreg_LC1) * np.ones(len(T2)), linestyle=':', linewidth=3, color='green', label=f'LocReg 1st Derivative')
        # plt.semilogy(T2, (lambda_locreg_LC2) * np.ones(len(T2)), linestyle=':', linewidth=3, color='crimson', label=f'LocReg 2ndDeriv {lam_ini_val}')
        plt.semilogy(T2, (lambda_locreg_LC2) * np.ones(len(T2)), linestyle=':', linewidth=3, color='orange', label=f'LocReg 2nd Derivative')
    ...
    if realresult == 1:
        ...
        feature_df["GT"] = [IdealModel_weighted]
        ...
        feature_df["err_upen0D"] = [err_upen0D]
        feature_df["err_upen1D"] = [err_upen1D]
        ...
        feature_df["upen_vect"] = [upen_zama_sol]
        feature_df["upen_vect1D"] = [upen_zama_sol1D]
        feature_df["upen_vect0D"] = [upen_zama_sol0D]
        feature_df["DP_lam"] = [lambda_DP]
        feature_df["LC_lam"] = [lambda_LC]
        feature_df["GCV_lam"] = [lambda_GCV]
        feature_df["upen_lam"] = [upen_zama_lams]
        feature_df["upen_lam1D"] = [upen_zama_lams1D]
        feature_df["upen_lam0D"] = [upen_zama_lams0D]
        ...
        # Add MSE, Bias, and Variance values to the DataFrame
        feature_df["MSE_LC"] = [MSE_LC]
        feature_df["Bias_LC"] = [bias_LC]
        feature_df["Var_LC"] = [var_LC]
        feature_df["MSE_DP"] = [MSE_DP]
        feature_df["Bias_DP"] = [bias_DP]
        feature_df["Var_DP"] = [var_DP]
        feature_df["MSE_GCV"] = [MSE_GCV]
        feature_df["Bias_GCV"] = [bias_GCV]
        feature_df["Var_GCV"] = [var_GCV]
        feature_df["MSE_oracle"] = [MSE_oracle]
        feature_df["Bias_oracle"] = [bias_oracle]
        feature_df["Var_oracle"] = [var_oracle]
        feature_df["MSE_LR"] = [MSE_LR]
        feature_df["Bias_LR"] = [bias_LR]
        feature_df["Var_LR"] = [var_LR]
        feature_df["MSE_LR1"] = [MSE_LR1]
        feature_df["Bias_LR1"] = [bias_LR1]
        feature_df["Var_LR1"] = [var_LR1]
        feature_df["MSE_LR2"] = [MSE_LR2]
        feature_df["Bias_LR2"] = [bias_LR2]
        feature_df["Var_LR2"] = [var_LR2]
        feature_df["MSE_upen"] = [MSE_upen]
        feature_df["Bias_upen"] = [bias_upen]
        feature_df["Var_upen"] = [var_upen]
        feature_df["MSE_upen0D"] = [MSE_upen0D]
        feature_df["Bias_upen0D"] = [bias_upen0D]
        feature_df["Var_upen0D"] = [var_upen0D]        
        feature_df["MSE_upen1D"] = [MSE_upen1D]
        feature_df["Bias_upen1D"] = [bias_upen1D]
        feature_df["Var_upen1D"] = [var_upen1D]
        ...
        # Add None for all error and vector fields
        feature_df["NR"] = [iter_sim]
        feature_df["Sigma"] = [sigma_i]
        feature_df["GT"] = [IdealModel_weighted]
        feature_df["RPS_val"] = [rps_val]
        ...
        feature_df["GCV_lam"] = [lambda_GCV]
        feature_df["upen_lam"] = [upen_zama_lams]
        feature_df["upen1D_lam"] = [upen_zama_lams1D]
        feature_df["upen0D_lam"] = [upen_zama_lams0D]
        feature_df["oracle_lam"] = [lambda_oracle]
        feature_df["LR_lam"] = [lambda_locreg_LC]
        feature_df["LR_lam_1stDer"] = [lambda_locreg_LC1]
        feature_df["LR_lam_2ndDer"] = [lambda_locreg_LC2]
...
#         vmax1 = np.max([maxLC, maxGCV, maxDP, maxoracle, maxupen, maxLR2])
...
#     add_heatmap(axs[4], compare_LR2_upen, tick_labels[4], title = f'LocReg2ndDeriv Error - UPEN Error ({err_type})\n' + f'Average UPEN Comparison Score: {avg_LR2_upen}',  x_ticks = x_ticksval, y_ticks = y_ticksval)
...
def indiv_heatmap():
    ...
    # vmax1 = np.max([maxLC, maxGCV, maxDP, maxoracle, maxUPEN, maxLR2UPEN])
