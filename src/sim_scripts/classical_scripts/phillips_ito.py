...
if np.any(g) < 0:
    ...
    print("phillips is non-neg problem")
...
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Phillips Problem No Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
...
#         table.set_fontsize(12)
#         table.scale(1.2, 2.5)
#         table_ax.set_title('Phillips Problem with Feedback Table', fontsize=16, fontweight='bold', y=1.2)  # Adjust the y position
#         plt.tight_layout()
#         string = "Phillips"
#         file_path = create_result_folder(string, SNR)
...
# date = datetime.now().strftime("%Y%m%d")
# np.savetxt(f'/Users/steveh/Downloads/NIH 23-24/LocReg_Python/Experiments/classical_problems/Testing_Ito_{date}_phillips.csv', vecs , delimiter=',')
...
np.savetxt('phillips_vec_1023.csv', vecs , delimiter=',')
# np.savetxt('phillips_noise_arr_1000', noise_arr)
...
plt.savefig(f'phillips_prob_nrun_{nrun}_{name}.png')  # You can specify the desired filename and format
...
plt.savefig(f'philips_prob_nrun{nrun}_09_11_23.png')  # You can specify the desired filename and format
plt.show()
"""