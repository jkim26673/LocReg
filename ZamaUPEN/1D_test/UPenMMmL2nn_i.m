function [xpwL2_nn,errML2,resML2,Q,INd_exit,ck,Lam,it_pn,time]=...
    UPenMMmL2nn_i(A,b,xex,noise_norm,beta_0,Kmax,tol_lam)
%
...
%         ck = 0.9*ck+0.1*ck_exact;
        ...
        Qk = (norm(A*x-b)^2 + x'*L2'*diag(ck)*L2*x + beta_0*sum(ck)) ...
            /  (prod(ck.^(1/(2*N))));
    end 
